...
unsigned long seed128[4] = {0}; // Declare seeds here
...
{			
	...
	void init_xor128(); // Initialize seeds for xor128()
...
void init_xor128() // Initialization
{
	unsigned long s = clock();
	unsigned long i = 0;
	for (i = 0; i < 4; i++)
	{
		seed128[i] = s = 1812433253U * (s^(s >> 30)) + i;
	}
}

unsigned long xor128() // Random number generator
{
