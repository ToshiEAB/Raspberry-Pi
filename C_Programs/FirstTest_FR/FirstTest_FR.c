...
/* Read 1 byte (1 character) from Arduino */
int ReadArduino(char *buf)
{
	int byte_count = 0;
	while (byte_count < 1)
	{
		byte_count = read(fd, buf, 1);
		if (byte_count == -1)
		{
...
void EndSession()
{
	...
	// Convert int to char
	sprintf(strdvResponse,      "Response:     %d\n", dvResponse);
	sprintf(strdvReinforcer,    "Reinforcer:   %d\n", dvReinforcer);
